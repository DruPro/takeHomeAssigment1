...
const props = defineProps(["currentItem","isModalActive","onQuantityClick","onExitClick","onBuyNow"])
...
<template>
  <div class="addToCartModal" v-show="props.isModalActive">
    <div class="headerContainer">
        <h1 class="modal-title">Buy Now</h1>
        <button class="exitButton" @click="onExitClick">Exit</button>
    </div>
    ...
    <div>
      <p>Total : {{ props.currentItem.price * props.currentItem.quantity }}</p>
    </div>
    <div class="buyNowButtonContainer">
        <CoolGlowyButton
        :button-action="props.onBuyNow"
        button-text="Buy Now"
        ></CoolGlowyButton>
    </div>
  </div>
